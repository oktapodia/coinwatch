import { BrowserWindow, ipcMain, Tray as TrayElectron, app } from 'electron';
...
class Tray {
  tray = new TrayElectron(getAssetPath('icon_16@2x.png'));
  ...
  onTrayUpdate = (_event: any, coins: ICoin[]) => {
    ...
    const trayDisplay = map(coinsFiltered, ({ coin, to, trend, price }) => {
      console.log(
        ...
        trend,
        price,
        `${coin.symbol}: ${formatPrice(price, to)} ${trend}`
      );
